...
#' @export
#' @examples
#' CKD.TOST(test.name="Biosimilar", ref.name="Reference", method.name="WAX-HPLC", ci=0.95, paired = FALSE )
#'
...
ckd.tost <- function(filename, sheet.no=1, raw.data, test.name, ref.name, method.name, ci=0.90, paired = FALSE, sample.n.show=TRUE){
  ...
  ggplot() +
    ...
    coord_flip()
  ggsave(filename = paste0("equivalent_test ", method.name, "_", test.name, " vs ", ref.name, ".png"), width = 10, height = 5 )
